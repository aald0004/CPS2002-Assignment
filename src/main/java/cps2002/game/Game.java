...
public class Game {
    ...
    // determines if the game will be played in team mode
    boolean teamMode;

    // stores the number of teams
    int numOfTeams;

    // Array list to store observers
    ...
    public void startGame(Scanner sc){
        ...
        for(int i = 0; i < numOfPlayers; i++){
            ...
            if(teamMode == true) {
                subject.setTeamID(players.get(i).getTeamID());
                subject.setState(players.get(i).position.getx(), players.get(i).position.gety(), 'G');
            }
        }
    ...
    /* ask the user if he wants to play in team mode
    * Parameters: scanner-> used to read user input*/
    public void setTeamMode(Scanner scanner){

        // ask user to decide whether to play in team mode
        System.out.println("Would you like to play in team mode: Yes or No");
        String mode = scanner.nextLine();

        // while user keeps inputting an invalid input, repeat the process
        while(mode.equalsIgnoreCase("YES") == false && mode.equalsIgnoreCase("NO") == false){
        ...
        if(mode.equalsIgnoreCase("Yes")){
            ...
            String playerPerTeam = scanner.nextLine();

            // catch an error if the user enters a character not a digit
            try {
                numOfTeams = Integer.parseInt(playerPerTeam);
            } catch(Exception e){
            ...
            // while user keeps inputting an invalid input, repeat the process
            while(validNumberOfTeams(numOfTeams) == false){

                System.out.println("Invalid number of teams. Must be between 2 and " + (numOfPlayers-1));
                System.out.println("How many teams: ");
                playerPerTeam = scanner.nextLine();
    ...
    /* Determine whether the number of teams inputted is valid or not
    * Parameters: number-> the number of teams
    * Returns: boolean-> true if valid, false if invalid*/
    public boolean validNumberOfTeams(int number){

        boolean isValid = false;

        // check if the number of teams is between 2 and the number of players-1
        if(numOfPlayers>2) {
            if (number >= 2 && number <= numOfPlayers - 1) {
                isValid = true;
            }
        } else {
            if(number == 2){
    ...
    /* choose the map type
    * Parameters: scanner-> used to read user input*/
    public void chooseMapType(Scanner scanner){

        // ask the user to input map type
        System.out.println("Choose map type: Safe or Hazardous");
        String type = scanner.nextLine();

        // keep requesting user to input map type until a valid input is provided
        while(type.equalsIgnoreCase("SAFE") == false && type.equalsIgnoreCase("HAZARDOUS") == false){
    ...
    public void chooseMapSize(Scanner scanner){
        ...
        tm = creator.create(mapType, numOfPlayers);
        ...
        // catch an error if the user enters a character and not a digit
        try {
    ...
    * Returns: Position-> player's starting position*/
    public Position startingPos(){
        ...
        x = rand.nextInt(tm.getSize() - 1);
        y = rand.nextInt(tm.getSize() - 1);
    ...
    /* fill the observers array with an observer for each team */
    public void setObservers(){
        ...
        for(int i = 0; i < numOfTeams;i++){
            ...
            for(int j = 0; j < numOfPlayers; j++){
                if(players.get(j).getTeamID() == i){
                    teamMembers.add(players.get(j));
                }
            }
    ...
    public void addPlayers(){
        ...
        for(int i = 0; i < numOfPlayers; i++){
            ...
            Player p = new Player(tm.getSize(),numOfPlayers, mapType);
    ...
    *             index-> the index in the players array list of the current player*/
    ...
    public void revealColour(int x, int y, int index){
        ...
        if(tileType == 'g'){
            players.get(index).revealColour(x,y,'G');
            if(teamMode == true) {
                subject.setTeamID(players.get(index).getTeamID());
                subject.setState(x, y, 'G');
            }

        } else {
            players.get(index).revealColour(x,y,tileType);
    ...
    public String htmlString(int num) {
        ...
        for(int i = 0; i < tm.getSize(); i++){
            ...
            for(int j = 0; j < tm.getSize(); j++){
                ...
                if(players.get(num).getTileType(j,i) == 'g'){
                    tileType = "greyTile";
                } else if(players.get(num).getTileType(j,i) == 'w'){
                    tileType = "waterTile";
                } else if(players.get(num).getTileType(j,i) == 't'){
